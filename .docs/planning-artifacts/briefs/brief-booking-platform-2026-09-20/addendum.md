...
| Booking horizon | A guest can book a time window only if it starts within a period the administrator sets, 4 weeks ahead by default. Windows further ahead are not yet offered to guests. [ASSUMPTION: the period is counted from the moment of booking to the start of the window, so new windows become bookable day by day; it applies to individual seats, whole saunas, period-card bookings and changing a booking into a later window; the administrator can still create and see windows further ahead.] |
| Seat hold | Seats are held and hidden from other guests while a booking is in progress, for a fixed 10 minutes, and released if the booking is not completed. The hold time is not an administrator setting: it depends on how long payment takes, not on the venue, and a wrong value would raise the risk of overbooking. The hold time does not by itself prevent double booking. The platform must check and reserve capacity in one atomic operation, both when seats are held and when a booking is confirmed, so concurrent bookings can never take the same seats. A payment completed after the hold has expired gives a booking only if the seats are still free; otherwise the payment is refunded. |
...
| Cancellation and change | Individual seats are not offered a cash refund. Up to 12 hours before the window starts, a guest can either change individual seats (one at a time, for example 1 of 4, or all together, e.g. to a different time window), or, on that same notice, choose a voucher (tilgodebevis) issued by email instead of changing. Capacity is restored either way and the administrator sees it. A whole-sauna booking can be cancelled or changed to a different time up to 72 hours before the window starts. On cancellation, the guest chooses a simulated refund or a voucher (tilgodebevis) issued by email, which expires one year after issue. Capacity is restored, and the administrator sees the cancellation or change. The administrator can change both cut-offs, 12 hours for individual seats and 72 hours for a whole sauna, to another time, but never to less than 2 hours before the window starts. |
| Door codes | Door codes apply only to saunas with self-service door locks. The administrator marks per sauna whether it has one; a sauna without one gets no codes, and a booking that only gives access to such saunas gets no door-code email. [ASSUMPTION: the fictional venue has self-service locks on all its saunas, so the door-code rules can be shown and tested; with one-sauna access, a booker whose sauna has no self-service lock is still told which sauna it is, by email at the same time, without codes.] At a time the administrator chooses before a booked window starts (20 minutes by default), the booker receives an email with the codes that unlock the doors they may use. Simulated in the first version: no real lock is connected, and the email is simulated like all other email. Each door has its own code per time window, so a sauna with more than one door can have a different code on each. [ASSUMPTION: the administrator registers the doors and which sauna each one belongs to.] The codes come from one of two sources: the administrator enters them, or the platform fetches them from a connected lock system. Simulated in the first version: no real lock system is connected, and the simulated lock system supplies the codes. [ASSUMPTION: the administrator chooses the source per door; a code entered by the administrator can be changed until the door-code email is sent; if a code is missing when the email is due, the administrator is alerted, and the email is sent as soon as the code is in place.] A code is shared by everyone who may use that door in that window, and is valid from the send time until the window ends. A whole-sauna booking gets the codes for that sauna. An individual-seat booking gets the codes only for the saunas open for individual seats in that window: not booked in full, not held back as whole-booking-only, and, with gradual opening on, already opened. Seat guests move freely between those saunas and are not told which one their seats are placed in. If another sauna opens for individual seats after the codes are sent (for example with gradual opening), those bookers get an extra email with its codes. One-sauna access (optional, set by the administrator, who can turn it on and off): seat guests may not move between saunas, so an individual-seat booking gives access to one sauna only, and all seats in the booking are placed in the same sauna. [ASSUMPTION: seats go to the first open sauna in the fill order with room for the whole booking; with gradual opening, the next sauna opens if no open sauna has room; seat moves after a cancellation move whole bookings, never part of one.] The door-code email then names that sauna and gives only its codes. If the seats are moved to another sauna after the email is sent, the booker gets a new email with the new sauna and its codes, and the extra email for newly opened saunas is not sent. A booking made after the send time gets the email at once. If a booking later loses to the overbooking fallback, the apology email also says the door codes must not be used. The administrator can see and resend the codes. [ASSUMPTION: one email per booking, sent to the booker, who shares the codes with their group.] Remote door control: where the lock allows it, the administrator can put a door in open mode, so it opens without a code, quick-open it once, for example to let a guest in, and lock it again at any time. A door in open mode is locked automatically when the window is over. By default, a door put in open mode outside a window is locked automatically when it closes; the administrator can change this rule. Simulated in the first version, like the locks themselves. [ASSUMPTION: the administrator marks per door whether its lock supports remote control; with the rule changed, a door put in open mode outside a window stays open until the administrator locks it; locking a door does not change its code; the platform records who opened or locked a door and when.] |
...
| Period cards | Optional, off by default; the administrator turns them on and off. The administrator sets the card periods offered (for example one month) and their prices. A guest buys a period card with the ordinary (simulated) payment and, while it is valid, books individual seats with no extra payment. Card bookings follow the ordinary booking rules: capacity, seat hold, booking deadline, seat placement and door codes. The administrator sets how many future bookings a cardholder can hold at once, meaning card bookings whose window has not yet started; one by default. The limit keeps the card from being used to block many seats at no cost. The administrator also sets how many period cards can be sold per period; 50 by default. Renewals come first: automatic renewals of cards from the previous period take their places before any new card is sold, and new cards are sold only from the places left. When the limit is reached, no more period cards are sold for that period. The limit applies to all card types together, and counts every card valid in the period, renewed or new. Periods follow the calendar, so a monthly card runs for a calendar month and all monthly cards share the same periods. Renewals always go through, also if the administrator has lowered the limit below the number of renewals (then no new cards are sold), unless the administrator chooses that renewals must also fit within the limit. If a limit across all card types together turns out not to be possible, the administrator must offer only one card type at a time, for example only monthly cards. [ASSUMPTION: with card types of different length, a card counts toward the limit in every calendar month it covers, so a three-month card takes a place in each of its three months; if renewals must fit within the limit and there are not enough places, the cards that have been held longest are renewed first, and the others get an email that their card was not renewed.] A period card is not valid for whole-sauna bookings, and it is not refunded. [ASSUMPTION: a period card is tied to a guest account and the cardholder must be logged in to use it; it covers the cardholder's own seat, one per time window, and other seats in the same booking pay the ordinary price; it covers windows that start while the card is valid; a card booking can be changed or cancelled up to the change cut-off for individual seats, but gives no voucher, since nothing was paid for it.] Renewal and payment: see Renewal and payment of memberships and period cards. |
...
| Time windows | Set freely by the administrator. New windows get a standard length, 1 hour 30 minutes by default (for example 15:00 to 16:30). The administrator can set another standard length. [ASSUMPTION: the administrator can also give a sauna or a single window another length than the standard; changing the standard length does not change windows already created.] |
...
| Setting | Default |
|---|---|
| Standard length of a time window | 1 hour 30 minutes; the administrator can set another standard length |
| How far ahead a guest can book | Up to 4 weeks before the window starts |
| Last time a window can be booked | 5 minutes after the window starts |
| Deadline to change individual seats or take a voucher | 12 hours before the window starts (no less than 2 hours) |
| Deadline to cancel or change a whole-sauna booking | 72 hours before the window starts (no less than 2 hours) |
...
| Remote door control (open mode, quick-open, lock) | Set per door by the administrator, where the lock allows it; a door in open mode is locked automatically when the window is over |
| Door put in open mode outside a window | Locked automatically when it closes (alternative: stays open until the administrator locks it) |
| Source of door codes | Set per door: entered by the administrator or fetched from a connected lock system (simulated in the first version) |
...
| Period cards that can be sold per period | 50, all card types together (renewals first, new cards from the places left) |
| Renewals of period cards go through even above the limit | On |
