...
| Children | Under 14 years. A child uses one seat of capacity. Lower price than adults. |
| Pricing | Individual seats are priced per person (adult and child price); a whole sauna has one price. The per-person price is the same across saunas; the whole-sauna price is set per sauna. Applies to a time window (for example 15:00 to 16:30). Several windows can be booked, consecutive or not, together or as separate bookings one after another, paying for each window. |
| Seat hold | Seats are held and hidden from other guests while a booking is in progress. Proposed hold: 10 minutes, then released if not completed. |
