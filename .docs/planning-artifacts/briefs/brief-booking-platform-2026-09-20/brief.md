...
- **Adjustable rules:** the administrator can change the booking rules that may differ from one venue or website to another: the cut-offs for changing seats and for cancelling or changing a whole sauna, when the door-code email is sent, when seats open for individual booking, or whether they open at all, whether saunas open for individual seats gradually and how many at most, the child age limit and whether children have their own price, how long seats are held during booking, how long gift cards and vouchers stay valid, and whether a cancelled whole-sauna booking offers a refund, a voucher or both. The times and limits stated elsewhere in this brief are the defaults. A changed setting applies only to bookings made after the change; existing bookings keep the rules they were made under.
