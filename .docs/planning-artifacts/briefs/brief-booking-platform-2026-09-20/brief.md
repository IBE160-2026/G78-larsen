...
- **Door codes:** door codes apply only to saunas with self-service door locks, which the administrator marks per sauna; saunas without them get no codes. At a time the administrator chooses before the window starts (20 minutes by default), the booker receives an email with the codes that unlock the doors they may use (simulated, like the email itself); a booking made later than that gets the email at once. Each door has its own code per time window, shared by everyone who may use that door, and valid from the send time until the window ends. The codes are either entered by the administrator or fetched from a connected lock system (simulated in the first version). A whole-sauna booker gets the codes for that sauna. An individual-seat booker gets the codes for the saunas that are open for individual seats in that window, since seat guests move freely between them, and never for a sauna that is not open for individual seats. If another sauna opens for individual seats after the codes are sent, those bookers get an extra email with its codes. The administrator can instead choose that seat guests may not move between saunas, and change that back. Then an individual-seat booking gives access to one sauna only, and all its seats are placed in the same sauna: the email names that one sauna and gives only its codes, and if the seats are moved to another sauna after the email is sent, the booker gets a new email with the new sauna and its codes. The administrator can see and resend the codes. Where the lock allows it, the administrator can also put a door in open mode, quick-open it once, and lock it again. A door in open mode is locked automatically when the window is over. By default, a door put in open mode outside a window is locked automatically when it closes; the administrator can change this rule (simulated in the first version).
...
- **Membership and period cards (optional):** the administrator can choose to offer memberships, period cards, or both, and turn them off again. A guest can buy a membership for a period, for example a year, and book at discounted member prices while it is valid; the member price covers only the member, and others in the same booking pay the ordinary price. A member who books a whole sauna gets a member price for it by default; the administrator can turn that off. A guest can also buy a sauna period card, for example a monthly card, and book individual seats while it is valid with no extra payment; a period card is not valid for whole-sauna bookings. The administrator sets which periods are offered, what they cost, the member prices, and how many future bookings a period card can hold at once (one by default). Both renew automatically unless the administrator turns that off, and a guest can turn off renewal of their own in their account, keeping it until the paid period ends. Neither is refunded. Both are paid with the ordinary (simulated) payment and are tied to a guest account, so the platform recognises the member or cardholder at booking. [ASSUMPTION: an account is required for both.] By default they cannot be paid with a gift card or voucher; the administrator can allow it.
- **Cancellation and change:** individual seats are not offered a cash refund; up to 12 hours before the start, a guest can either change them (for example to a different time window) or choose a voucher sent by email instead of changing. A whole-sauna booking can be cancelled or changed to a different time up to 72 hours before the start; on cancellation, the guest chooses between a simulated refund and a voucher sent by email. The administrator can change both cut-offs, the 12 hours and the 72 hours, to another time, but never to less than 2 hours before the start.
- **Administration:** administrators have their own accounts. An administrator sets time windows freely, including their length, manages capacity and prices, sets the gift card denominations and the limit on buyer-chosen amounts, sets up memberships, member prices and period cards if they are offered, and can mark windows as whole-booking-only or reverse that. New windows get a standard length of 1 hour 30 minutes, and the administrator can set another standard length. If a sauna is not booked in full for a window, its seats open for individual booking at 06:00 the same morning. The administrator can move this to another time or turn the rule off.
- **Adjustable rules:** the administrator can change the booking rules that may differ from one venue or website to another: the standard length of a time window (1 hour 30 minutes by default), how far ahead a guest can book (4 weeks by default), the last time a window can be booked (5 minutes after the window starts by default), how a door put in open mode outside a window is locked, the cut-offs for changing seats and for cancelling or changing a whole sauna (no less than 2 hours before the start), when the door-code email is sent, when seats open for individual booking, or whether they open at all, whether saunas open for individual seats gradually and how many at most, whether seat guests may move between the open saunas or an individual-seat booking is kept to one sauna, the child age limit and whether children have their own price, how long seats are held during booking, how long gift cards and vouchers stay valid, whether memberships and period cards are offered, whether members get a member price for a whole sauna, how many future bookings a period card can hold, whether they renew automatically and whether they can be paid with a gift card or voucher, and whether a cancelled whole-sauna booking offers a refund, a voucher or both. The times and limits stated elsewhere in this brief are the defaults. A changed setting applies only to bookings made after the change; existing bookings keep the rules they were made under.
...
- The booking rules: individual-seat and whole-sauna bookings, several windows per guest, a booking horizon (4 weeks ahead by default), adult pricing and optional child pricing, a seat hold (10 minutes by default) and the overbooking fallback.
...
- Real door locks and lock systems, heater controls and temperature sensors; all are simulated.
